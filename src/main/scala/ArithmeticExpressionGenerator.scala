...
object ArithmeticExpressionGenerator {
  ...
  @tailrec
  def generateIntegerExpressions(max: Int, exprList: List[ArithExp[Int]]): List[ArithExp[Int]] = {
    if max > 0 then
      ...
      generateIntegerExpressions(max-1, exprList :+ new ArithExp[Int](exp1, op, exp2))
    else
      exprList
  }
}
